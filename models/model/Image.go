...
func (m *Image) LinkSmall() string {
	var width int
	switch {
	case m.Width > conf.ImageSizeSmall:
		width = conf.ImageSizeSmall
	}
...
func (m *Image) LinkMiddle() string {
	var width int
	switch {
	case m.Width > conf.ImageSizeMiddle:
		width = conf.ImageSizeMiddle
	}
	return m.LinkSize(width)
}
...
func (m *Image) LinkSize(width int) string {
	...
	var size string
	switch width {
	case conf.ImageSizeSmall, conf.ImageSizeMiddle:
...
func (m *Image) GetToken() string {
	...
	return utils.NumberEncode(number, conf.ImageLinkAlphabets)
}

func (m *Image) DecodeToken(token string) error {
	number := utils.NumberDecode(token, conf.ImageLinkAlphabets)
	if len(number) < 9 {
