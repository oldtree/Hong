...
func SaveImage(m *model.Image, r io.ReadSeeker, mime string, filename string, created time.Time) error {
	var ext string
	...
	if ext != ".gif" {
		...
		if m.Width > conf.ImageSizeSmall {
			if err := ImageResize(m, img, conf.ImageSizeSmall); err != nil {
				os.RemoveAll(fullPath)
				return err
			}
		}

		if m.Width > conf.ImageSizeMiddle {
			if err := ImageResize(m, img, conf.ImageSizeMiddle); err != nil {
				os.RemoveAll(fullPath)
				return err
			}
		}
...
func ImageResize(img *model.Image, im image.Image, width int) error {
...
func GenImagePath(img *model.Image) string {
	return "upload/img/" + beego.Date(img.Created, "y/m/d/s/") + utils.ToStr(img.Id) + "/"
}

func GenImageFilePath(img *model.Image, width int) string {
	var size string
	if width == 0 {
		size = "full"
	} else {
